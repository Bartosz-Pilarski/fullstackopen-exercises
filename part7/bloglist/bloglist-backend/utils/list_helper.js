...
const mostBlogs = (blogs) => {
  ...
  blogs.forEach((blog) => {
    const existing = bloggers.findIndex((blogger) => {
      return blog.author === blogger.author
    })
    if (existing === -1)
      return (bloggers = bloggers.concat({ author: blog.author, blogs: 1 }))

    bloggers[existing] = {
      author: bloggers[existing].author,
      blogs: (bloggers[existing].blogs += 1),
    }
  })
  ...
  bloggers.forEach((blogger) => {
    if (blogger.blogs >= topBlogger.blogs) topBlogger = blogger
  })
...
const mostLikes = (blogs) => {
  ...
  const authorsByLikes = Object.values(authors).map((authorPosts) => {
    return {
